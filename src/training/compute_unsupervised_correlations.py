...
def compute_unsupervised_correlation(dataset: str) -> Tuple:
    ...
    # Load dataset
    for i in vals:
        df_elbo = pd.read_csv(f"data/interim/{dataset}/{dataset}_EVE_ELBO_{i}.csv", index_col=0)
    ...
    mu = np.mean(corrs)
    df_ll = pd.read_csv(f"data/interim/{dataset}/esm_if1_likelihoods.csv", index_col=0)
    df = pd.merge(left=df, right=df_ll, on="name", how="left")
    ll, _ = spearmanr(df[target].values, df["ll"].values)

    df_t = pd.read_csv(f"data/interim/{dataset}/{dataset}_tranception_scores.csv", index_col=0)
